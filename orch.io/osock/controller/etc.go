...
func AdminRequest(email string, query string) ([]byte, error) {
	...
	case "ORCH-INSTCL":
		...
		go models.InstallCluster(session_key, cluster_id, targetip, targetid, targetpw, localip, osnm, cv, update_token)
	...
	case "ORCH-INSTCLLOG":
		...
		log_b, err := models.InstallClusterLog(session_key, cluster_id, targetip, targetid, targetpw)
